...
int main(int argc, char** argv) {
    ...
    int sockClient, sockServeur;
    int valEnvoyer,valRecue;
    int retour;
    ...
    retour = write(sockServeur,(void *) &valEnvoyer, sizeof(&valEnvoyer));
    if (retour == -1){
        printf("pb write : %s\n", strerror(errno));
        exit(errno);
    }
    ...
    retour = read(sockServeur, (void *) &valRecue, sizeof(&valRecue));
    if (retour == -1){
        printf("pb read : %s\n", strerror(errno));
        exit(errno);
    }
