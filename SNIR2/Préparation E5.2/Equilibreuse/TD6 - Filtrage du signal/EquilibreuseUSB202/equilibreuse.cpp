...
{
    double dephasageFiltre;
    double attenuationFiltre;
    double x;
    double y;
    attenuationFiltre = ((-0.000001*1620)/60.0)+0.75443;
    dephasageFiltre = ((-1.53311*1620)/60.0)-16.91160;
    ...
    else
    {
        if(status == SS_RUNNING)
        {
            ...
            for (int i=0;i<index;i+=NB_CANAUX) {
                x= (i*360)/1000.0;
                courbeA << QPointF(x,valeursBrutes[i+512]);
                //compensation du filtre
                int indice = i + 513 -floor(dephasageFiltre);
                y = valeursBrutes[indice]/attenuationFiltre;
